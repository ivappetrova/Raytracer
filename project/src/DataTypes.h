...
#pragma region GEOMETRY
	...
	struct TriangleMesh final
	{
		...
		void CalculateNormals()
		{
			...
			for (size_t triangleIndex{}; triangleIndex < NUM_TRIANGLES; ++triangleIndex)
			{
				...
				const Vector3 N = Vector3::Cross(EDGE1, EDGE2).Normalized();
		...
		{
			const Matrix FINAL_TRANSFORM { rotationTransform * translationTransform * scaleTransform };
			...
			{
				transformedPositions.emplace_back(FINAL_TRANSFORM.TransformPoint(pos));
			}
			UpdateTransformedAABB(FINAL_TRANSFORM);

			transformedNormals.clear();
			transformedNormals.reserve(normals.size());

			const Matrix NORMAL_TRANSFORM{ rotationTransform };
			for (const auto& n : normals)
			{
				transformedNormals.emplace_back(NORMAL_TRANSFORM.TransformVector(n).Normalized());
			}
		}
